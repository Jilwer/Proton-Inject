...
# debug: cross-compile Rust artifacts, refresh embedded/assets, then go build (no strip).
...
(cd "$LOADER_DIR" && cargo build --target x86_64-pc-windows-gnu)
...
echo "Building injector (debug) ..."
(cd "$INJECTOR_DIR" && cargo build --target x86_64-pc-windows-gnu)
...
echo "Building $APP_NAME v$VERSION (debug) ..."
go build -ldflags="-X main.version=${VERSION}" -o "$OUT_DIR/$APP_NAME" .
